...
export class ColorFilterService {
  ...
  css(filters:any) {
    function fmt(idx = 0, multiplier = 1) {
  ...
  loss(filters:any) {
    ...
    const color = new ColorFilterService();
    color.set(0, 0, 0);
    ...
    return (
      ...
      Math.abs(colorHSL.h - this.hsl().h) +
      Math.abs(colorHSL.s - this.hsl().s) +
      Math.abs(colorHSL.l - this.hsl().l)
    );
  }

  spsa(A:number, a:any[], c:number, values:any[], iters:any) {
    const alpha = 1;
